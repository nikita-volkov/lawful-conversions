...
-- === Testing
--
-- For testing whether your instances conform to these laws use 'LawfulConversions.isSubsetOfProperties'.
class IsSubsetOf a b where
  -- |
  ...
  -- |
  -- Requires the presence of 'IsSubsetOf' in reverse direction.
  default maybeFrom :: (IsSubsetOf b a) => a -> Maybe b
  maybeFrom = Just . to
  ...
  --
  -- > fromString :: IsSubsetOf String b => String -> b
  ...
  -- |
  -- Requires the presence of 'IsSubsetOf' in reverse direction.
  default onfrom :: (IsSubsetOf b a) => a -> b
  onfrom = to
...
--
-- > fromText = from @Text
from :: forall b a. (IsSubsetOf a b) => b -> a
from = to
...
maybeTo :: forall b a. (IsSubsetOf a b) => a -> Maybe b
maybeTo = maybeFrom
...
-- @
onto :: forall b a. (IsSubsetOf a b) => a -> b
onto = onfrom
...
-- namely: @Is A B@ and @Is B A@, @IsSubsetOf A B@ and @IsSubsetOf B A@.
--
...
--
-- @
-- instance IsSubsetOf "Data.Text.Lazy.LazyText" "Data.Text.Text" where
--   to = LazyText.'Data.Text.Lazy.fromStrict'
--   onfrom = LazyText.'Data.Text.Lazy.toStrict'
--
-- instance IsSubsetOf "Data.Text.Text" "Data.Text.Lazy.LazyText" where
...
-- @
class (IsSubsetOf a b, Is b a) => Is a b
